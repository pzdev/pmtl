...
LSTMBuilder::LSTMBuilder(unsigned layers, unsigned shared_layers, LSTMBuilder *_p_obj,
		  unsigned input_dim, unsigned hidden_dim, Model &model) : layers(layers), input_dim(input_dim), hid(hidden_dim) {

	unsigned layer_input_dim = input_dim;

	  // local layers ------
	  for (unsigned i = 0; i < layers-shared_layers; ++i) {
	    // i
	    Parameter p_x2i = model.add_parameters({hidden_dim, layer_input_dim});
	    Parameter p_h2i = model.add_parameters({hidden_dim, hidden_dim});
	    Parameter p_c2i = model.add_parameters({hidden_dim, hidden_dim});
	  ...
	  // shared layers -----
	  for (unsigned i = layers-shared_layers; i < layers; ++i) {
		  params.push_back( _p_obj->params[i] );
	  }

	  dropout_rate = _p_obj->dropout_rate;// 0.f;
	  dropout_rate_h = _p_obj->dropout_rate_h;//  0.f;
	  dropout_rate_c = _p_obj->dropout_rate_c;// 0.f;
}
...
VanillaLSTMBuilder::VanillaLSTMBuilder(unsigned layers, unsigned shared_layers, VanillaLSTMBuilder *_p_obj,
                                       unsigned input_dim,
                                       unsigned hidden_dim,
                                       Model& model,
                                       bool ln_lstm) : layers(layers), input_dim(input_dim), hid(hidden_dim), ln_lstm(ln_lstm) {
  ...
  for (unsigned i = layers-shared_layers; i < layers; ++i) {
	  params.push_back( _p_obj->params[i] );
	  if (ln_lstm) ln_params.push_back(_p_obj->ln_params[i]);
  }

  dropout_rate = _p_obj->dropout_rate;
  dropout_rate_h = _p_obj->dropout_rate_h;
}
